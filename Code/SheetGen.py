...
def Whatformat(file_1):
    RandFormat = 'R#,##0.00'
    DollarFormat = '$#,##0.00'
    if "ltd" in file_1.lower() or 'limited' in file_1.lower():
        return DollarFormat
    else:
        return RandFormat
...
def makepivot(*args):   
    ...
    worksheet2_admin.set_column('A:AZ',30, formatwrap)
...
def QuarterMerge():
    ...
    try:
        for element in dff1.sheet_names[1::]:  
            ...
            dummy1.set_column('F:AZ', 35, formatf)
    ...
    try:
        for element in dff2.sheet_names[1::]:  
            ...
            dummy2.set_column('F:AZ', 35, formatf)
    ...
    try:
        for element in dff3.sheet_names[1::]:  
            ...
            dummy3.set_column('F:AZ', 35, formatf)
...
def Merge(**filestot):  
    ...
    worksheet_1.set_column('F:AZ',35,formatf)
...
def write(df_key,df_join,Advisor,Mgnt_admin,Perf_admin,file_1,Monthname,RMBSplit):
    ...
    worksheet1.set_column('F:AZ', 35, format1)
...
def Main(*args):
    ...
    try:
        ...
        master_worksheet.set_column('F:AZ', 18, master_format)
        # Dictate Advisor Split sheet beginning columns format (Size)
        master_worksheet.set_column('A:AZ', 30)
