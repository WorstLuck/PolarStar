...
def Main(file_1,file_2,file_3,sheet_name_admin,sheet_name_advisor,sheet_name_key,Investor_admin,Investor_advisor,
        Series_admin,Series_advisor,Mgnt_admin, Perf_admin,skiprows_admin,skiprows_advisor,skiprows_key,
         Advisor,Date,Range):
    try:
        print("Checking if a J- file exists for that admin file")
        df_join = pd.read_excel('J-' + str(file_1))
        print("J- file exists!")
        ...
        if df_fill.empty:
            try:
                ...
                write(df_key,df_join,Advisor,Mgnt_admin,Perf_admin,Date)
            except:
                print("ERROR: One of {}, {} are spelled wrong".format(Mgnt_admin,Perf_admin))
        else:
            print("Warning! Not all advisors have been assigned in {} file!".format('J-' + str(file_1)))
    except:
        print("Warning! not all advisors assigned - please do it manually on test file and press receive file again")
        ...
        #Unfilled things
        df_join.to_excel('J-' + str(file_1),sheet_name= 'Joined')
    
def write(df_key,df_join,Advisor,Mgnt_admin,Perf_admin,Date):
    #Display database
